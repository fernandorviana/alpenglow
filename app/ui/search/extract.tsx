 ...
 * query could match. Except across a `<wbr>`: it offers a line break inside
 * one word — `alpenglow/<wbr>tokens.css` — and the word is still one, so the
 * text on either side joins without a space. The constants are
 * NodeFilter.SHOW_ELEMENT and SHOW_TEXT; the script's Node has no NodeFilter
 * global.
 */
function text(el: Element): string {
  const walker = el.ownerDocument.createTreeWalker(el, 1 | 4);
  const parts: string[] = [];
  let joined = false;
  for (let node = walker.nextNode(); node; node = walker.nextNode()) {
    if (node.nodeType === 1) {
      if ((node as Element).tagName === 'WBR') joined = true;
      continue;
    }
    ...
    if (joined && parts.length > 0) parts[parts.length - 1] += node.nodeValue ?? '';
    else parts.push(node.nodeValue ?? '');
    joined = false;
  }
