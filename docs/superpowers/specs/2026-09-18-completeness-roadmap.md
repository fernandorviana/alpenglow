...
| **Table responsive columns** — built 2026-09-24, on main 2026-09-24 as `192aec1`, unreleased | Table, Button | Columns shrink to a minimum and leave by priority as the Table's own width shrinks, whether the window or a side panel narrows it; the row's actions gather into "⋯" first; the primary, selection and actions never leave. Replaces the 40rem collapse to a list. **What moved up:** the Button's icon-only form. Spec `2026-09-24-table-responsive-columns-design.md`. |
