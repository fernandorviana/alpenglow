...
The reference implementation is the audit's throwaway harness, now `scripts/audit` (its `capture.mjs` and `cssorder.mjs`), split into the four files above, with three changes: routes come from `out/**/index.html` (not a hard-coded list), Chrome's path comes from `CHROME` or the macOS default, and the profile directory is a temp dir removed on exit.
